        ...
        if ($type === 'register') {
            ...
            if (!$user_id) {
                ...
                if ($user && verifyOTP($user['email'], $submittedOTP)) {
                    ...
                    $_SESSION['is_admin'] = $is_admin;   // 🔥 must exist BEFORE logging
                    unset($_SESSION['temp_user_id'], $_SESSION['temp_is_admin']);
                    // ================= Track Active Session =================
$stmt = $pdo->prepare("
    INSERT INTO user_sessions (user_id, session_id, last_activity, ip_address, user_agent)
    VALUES (?, ?, NOW(), ?, ?)
");
$stmt->execute([
    $_SESSION['user_id'],
...
logAdminAction(
    ...
    $user_id,
    'Login',
    "$role logged in successfully with OTP verification"
);
    ...
    $user_id,
    'Transfer',
    'Transferred ₹' . $transfer['amount'] . ' to account ' . $transfer['to_account']
);
    ...
    $user_id,
    'withdrawl',
    'Withdrawl ₹' . number_format($withdraw['amount'], 2)
);
            ...
            if (!isset($_SESSION['pending_deposit'])) {
                ...
                if ($user && verifyOTP($user['email'], $submittedOTP)) {
                    try {
                      ...
                      // 🔐 AUDIT LOG ENTRY — ADD THIS BLOCK
logAdminAction(
    $pdo,
    $user_id,
    'Deposit',
    'Deposited ₹' . number_format($deposit['amount'], 2)
);
